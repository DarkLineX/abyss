...
intptr_t SvcerSyscall::Call(int nr,
                       intptr_t p0,
                       intptr_t p1,
                       intptr_t p2,
                       intptr_t p3,
    ...
    static_assert(sizeof(void*) == sizeof(intptr_t), "pointer types and intptr_t must be exactly the same size");
...
#elif defined(__mips__)
    intptr_t err_status;
    intptr_t ret = SvcerSyscall::SandboxSyscallRaw(nr, args, &err_status);
    if (err_status) {
...
void SvcerSyscall::PutValueInUcontext(intptr_t ret_val, ucontext_t* ctx) {
#if defined(__mips__)
...
#if defined(__mips__)
intptr_t SvcerSyscall::SandboxSyscallRaw(int nr, const intptr_t* args, intptr_t* err_ret) {
